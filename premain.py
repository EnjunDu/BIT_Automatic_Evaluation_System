...
if __name__ == '__main__':
    ...
    PASSWORD = ''
    teacher_num = 0
    print(f"[INFO]|{current_time()}|百丽宫自动评教系统——updated by sky")
    ACCOUNT = input(f"[INFO]|{current_time()}|请输入学号：")
    PASSWORD = input(f"[INFO]|{current_time()}|请输入密码:")
    ...
    print(f"[INFO]|{current_time()}|密码:{blue('*' * len(PASSWORD))}")
